...
function handleFPSText() {
  try {
    ...
    if (!statsView) return; // если нет элемента — ничего не делаем

    // по-умолчанию показываем (если ключа в localStorage нет, считаем показ включённым)
    const stored = localStorage.getItem("show_fps");
    const showFPS = stored === null ? "1" : stored;

    // Устанавливаем видимость (оставляем стиль пустым для нормального рендеринга)
    if (showFPS === "1") {
      statsView.style = "";
    } else {
      statsView.style = "opacity: 0 !important; pointer-events: none;";
    }

    // Обработчик касания/клика — переключает show_fps в localStorage и обновляет стиль
    // Останавливаем всплытие, чтобы клик по статистике не ломал другие обработчики
    statsView.ontouchstart = statsView.onclick = function (e) {
      e && e.stopPropagation && e.stopPropagation();

      const cur = localStorage.getItem("show_fps") || "1";
      if (cur === "1") {
    ...
    // Удобная утилита для чтения текущего текста статы (FPS/Ping) при необходимости
    // Возвращает строку с текстом внутри .stats-view или null при ошибке
    window.getGameStatsText = function () {
      try {
        return statsView.textContent || statsView.innerText || null;
      } catch (err) {
        return null;
      }
    };
  } catch (err) {
    // если iframe неожиданно не доступен (cross-origin) или иная ошибка — молча логируем и выходим
    console.warn("handleFPSText error:", err);
  }
  }
