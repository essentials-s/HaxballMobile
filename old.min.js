...
(function(){
  if (window.__hb_fps_ping_mobile) {
    // toggle visibility if already injected
    const o = window.__hb_fps_ping_mobile;
  ...
  window.__hb_fps_ping_mobile = {
    el,
    stop: cleanup
  };

  // FPS loop (EMA smoothing)
  ...
  // Ping via Image load (works on mobile better than fetch/CORS)
  function measurePingOnce(timeout = 3000) {
    return new Promise((resolve) => {
      // Try same-origin favicon first (fast if available), fallback to public small image
      ...
      function tryCandidate(url) {
        ...
        try {
          ...
          // if src assignment throws, fallback to next
          if (tried < candidates.length) tryCandidate(candidates[tried]);
          else resolve(null);
        }
      }
  ...
  function startPingLoop() {
    async function run() {
      const ms = await measurePingOnce(3000);
      if (ms === null) {
        // could not measure
        state.ping = null;
      } else {
    ...
    // measure immediately, then every 2s
    run();
  ...
  // Detect "in-game" robustly for mobile:
  // - try .game-view in page or inside same-origin iframe .gameframe
  // - if not available, fallback to presence of a canvas > small size
  function isInGame() {
    try {
      if (document.querySelector('.game-view')) {
        // ensure not a roomlist view
        if (document.querySelector('.roomlist-view') && !document.querySelector('.game-view')) return false;
        return true;
      }
    } catch (e){}
    // try iframe named .gameframe (same-origin)
    try {
    ...
    // fallback: if there's a visible canvas likely game is running
    try {
  ...
  // Auto show/hide logic using MutationObserver + interval (for mobile reliability)
  ...
  function cleanup() {
    ...
    try { el.remove(); } catch {}
    delete window.__hb_fps_ping_mobile;
  }

  // start
  startAutoToggle();

  // If already in game, show immediately
  if (isInGame()) {
    el.style.display = 'block';
    state.running = true;
    state.lastFrame = performance.now();
  ...
  // export for debug/control
  window.__hb_fps_ping_mobile._state = state;
})();
