...
(function(){
  if (window.__hb_fps_ping_mobile_v2) {
    // toggle visibility if already injected
    const o = window.__hb_fps_ping_mobile_v2;
    o.el.style.display = o.el.style.display === 'none' ? 'block' : 'none';
    return;
  }
  ...
  window.__hb_fps_ping_mobile_v2 = {
    el,
    stop: cleanup
  };

  function fpsStep(now) {
    const dt = now - state.lastFrame;
    state.lastFrame = now;
    const inst = dt > 0 ? 1000 / dt : 0;
    const alpha = 0.12;
    state.fpsEMA = state.fpsEMA ? (alpha * inst + (1 - alpha) * state.fpsEMA) : inst;
    state.fps = Math.round(state.fpsEMA);
    updateText();
    state.rafId = requestAnimationFrame(fpsStep);
  }

  // Accurate ping measurement for mobile:
  // - use navigator.connection.rtt if available (fast hint)
  // - otherwise do a warmup request then N image probes, take best/median of fastest to avoid handshake overhead
  async function measurePingAccurate(options = {}) {
    const timeout = options.timeout || 2500;
    const probes = options.probes || 6;
    const warmups = options.warmups || 1;

    // If Network Information API gives rtt, use it as quick hint (but it's not always accurate)
    try {
      const nav = navigator;
      if (nav && nav.connection && typeof nav.connection.rtt === 'number' && nav.connection.rtt > 0) {
        // Use it but still perform active probes in background
        const hint = Math.round(nav.connection.rtt);
        // Start background probes but return hint quickly
        measurePingBackground();
        return hint;
      }
    } catch (e) {}

    // Try to measure actively
    ...
    const candidates = [
      ...
      'https://www.google.com/images/clear.gif'
    ];

    // Helper: single image probe
    function probeImage(url, to) {
      return new Promise((resolve) => {
        ...
        img.onload = img.onerror = function() {
          ...
          clearTimeout(timer);
          const dt = Math.max(0, Math.round(performance.now() - start));
          resolve(dt);
        };
        // cache bust
        try {
          img.src = url + (url.indexOf('?') === -1 ? '?_hb=' : '&_hb=') + Date.now();
        } catch (e) {
          clearTimeout(timer);
          resolve(null);
        }
    ...
    // Warmup to avoid one-off handshake cost (do 1..warmups quick attempts to same-origin)
    for (let w = 0; w < warmups; w++) {
      try {
        await probeImage(candidates[0], timeout);
      } catch (e) {}
    }

    // Now do sequential probes (sequential to avoid parallel queueing and get accurate timings)
    const results = [];
    for (let i = 0; i < probes; i++) {
      const url = candidates[i % candidates.length];
      /* eslint-disable no-await-in-loop */
      const r = await probeImage(url, timeout);
      results.push(r);
      await new Promise(rp => setTimeout(rp, 120));
    }

    // Filter out nulls and sort ascending
    const good = results.filter(v => typeof v === 'number').sort((a,b) => a-b);
    if (good.length === 0) return null;

    // Take the median of fastest half (to ignore outliers)
    const take = Math.max(1, Math.floor(good.length / 2));
    const fastest = good.slice(0, take);
    const sum = fastest.reduce((s,v) => s+v, 0);
    const avg = Math.round(sum / fastest.length);
    return avg;
  }

  // Start background probes but don't block UI when navigator.connection.rtt used
  function measurePingBackground() {
    (async () => {
      try {
        const p = await measurePingAccurate({probes:6, warmups:1, timeout:2500});
        if (p !== null) {
          const alpha = 0.18;
          state.pingEMA = state.pingEMA ? Math.round(alpha * p + (1 - alpha) * state.pingEMA) : p;
          state.ping = state.pingEMA;
          updateText();
        }
      } catch (e) {}
    })();
  ...
  function startPingLoop() {
    async function run() {
      try {
        const ms = await measurePingAccurate({probes:6, warmups:1, timeout:2500});
        if (ms === null) {
          state.ping = null;
        } else {
          const alpha = 0.18;
          state.pingEMA = state.pingEMA ? Math.round(alpha * ms + (1 - alpha) * state.pingEMA) : ms;
          state.ping = state.pingEMA;
        }
        updateText();
      } catch (e) {
    ...
    run();
    state.pingIntervalId = setInterval(run, 2000);
  }
  ...
  function updateText() {
    const fpsStr = (state.fps || state.fps === 0) ? state.fps : '--';
    const pingStr = (state.ping || state.ping === 0) ? (state.ping + ' ms') : '--';
  ...
  function isInGame() {
    try {
      if (document.querySelector('.game-view')) {
        if (document.querySelector('.room-link-view')) return false;
        return true;
      }
    } catch (e){}
    try {
      const gf = document.querySelector('.gameframe');
      if (gf && gf.contentDocument) {
        if (gf.contentDocument.querySelector('.game-view')) return true;
      }
    } catch (e){}
    try {
  ...
  function startAutoToggle() {
    ...
    check();
    try {
      state.obs = new MutationObserver(check);
      state.obs.observe(document.body, { childList: true, subtree: true, attributes: false });
    } catch (e) {
      state.obs = null;
    }
    state.checkInterval = setInterval(check, 900);
  }
  ...
  function cleanup() {
    ...
    try { el.remove(); } catch {}
    delete window.__hb_fps_ping_mobile_v2;
  }
  ...
  window.__hb_fps_ping_mobile_v2._state = state;
})();
