...
function handleFPSText() {
    const statsElement = getByDataHook('stats');

    if (statsElement) {
        statsElement.classList.add('stats-view');
        statsElement.style = '';

        const observerStats = new MutationObserver((mutationsList, observer) => {
            const targetElement = mutationsList[0].target;
            const newText = targetElement.textContent;

            if (newText.includes('FPS')) {
                let [fpsText, pingText] = newText.split('|').map(item => item.trim());
                
                // Форматирование FPS (например, "FPS 60" -> "60 FPS")
                fpsText = fpsText.replace('FPS ', '') + ' FPS';
                // Форматирование пинга (например, "PING 15" -> "15 ms")
                pingText = pingText.replace('PING ', '') + ' ms';

                // Обновление текста с новой структурой
                statsElement.textContent = `${fpsText} | ${pingText}`;
            }
        });

        observerStats.observe(statsElement, {
            characterData: true,
            subtree: true,
            childList: true
        });
...
// Добавляем элементы для FPS и пинга
...
window.pingInterval = setInterval(() => {
  if (window.room) {
    ...
    window.room.send({ type: 'ping' });
    window.room.on('ping', () => {
      ping = Date.now() - start;
      statsDiv.innerText = `Ping: ${ping}ms | FPS: ${fps}`;
    });
  }
}, 1000);

// FPS
let fps = 0, frame = 0, lastTime = performance.now();
