...
(function() {
  // FPS + Ping overlay (pure JavaScript)
  // Usage: вставь этот код в консоль / через инжектор (InjecThor). Показывает FPS и сетевой ping (HTTP RTT к origin).
  // Ограничение: без доступа к window.room или к low-level сокетам, RTT измеряется по HTTP-запросу к window.location.origin — приближённый ping до сервера страницы, но не обязательно до игрового RealTime-сервера.

  if (window.__hb_fps_ping_overlay) {
    // если уже запущено — переключаем видимость
    window.__hb_fps_ping_overlay.el.style.display = window.__hb_fps_ping_overlay.el.style.display === 'none' ? 'block' : 'none';
    return;
  }
  ...
  // Create overlay element (inline styles only)
  const el = document.createElement('div');
  el.style.position = 'fixed';
  el.style.left = '50%';
  el.style.transform = 'translateX(-50%)';
  el.style.bottom = '24px';
  el.style.zIndex = 2147483647; // topmost
  el.style.pointerEvents = 'none';
  el.style.background = 'transparent';
  el.style.opacity = '0.6'; // as requested
  el.style.color = '#fff';
  el.style.fontFamily = 'Arial, sans-serif';
  el.style.fontSize = '12px'; // маленький текст
  el.style.lineHeight = '1';
  el.style.padding = '4px 8px';
  el.style.borderRadius = '4px';
  el.style.textShadow = '0 0 3px rgba(0,0,0,0.6)';
  el.style.userSelect = 'none';
  el.style.display = 'none'; // показываем только в игре

  el.textContent = 'FPS: -- | PING: -- ms';
  document.body.appendChild(el);

  // Export handle for toggling or stopping later
  window.__hb_fps_ping_overlay = {
    el,
    stop: () => {
      state.running = false;
      if (state.rafId) cancelAnimationFrame(state.rafId);
      if (state.pingIntervalId) clearInterval(state.pingIntervalId);
      if (state.checkIntervalId) clearInterval(state.checkIntervalId);
      el.remove();
      delete window.__hb_fps_ping_overlay;
    }
  };

  // FPS measurement (smoothed)
  function fpsLoop(now) {
    // Compute instantaneous FPS based on time difference
    const dt = now - state.lastFrameTime;
    state.lastFrameTime = now;
    const instFps = dt > 0 ? (1000 / dt) : 0;
    // Exponential moving average for stability
    const alpha = 0.12;
    state.fpsSmoothed = state.fpsSmoothed ? (alpha * instFps + (1 - alpha) * state.fpsSmoothed) : instFps;
    state.fps = Math.round(state.fpsSmoothed);
    updateText();
    state.rafId = requestAnimationFrame(fpsLoop);
  }

  // Ping measurement via fetch to origin with cache-busting
  async function measurePingOnce() {
    const url = window.location.origin || (window.location.protocol + '//' + window.location.host);
    // choose a lightweight endpoint: try /favicon.ico then / (fallback)
    const candidates = ['/favicon.ico', '/play', '/'];
    let lastError = null;
    for (let path of candidates) {
      const full = url + path + '?_hb_ping=' + Date.now();
      const t0 = performance.now();
      try {
        // Try HEAD first for minimal response; if server disallows HEAD, fallback to GET
        let ok = false;
        try {
          await fetch(full, { method: 'HEAD', cache: 'no-store', mode: 'cors' });
          ok = true;
        } catch (e) {
          // HEAD failed; try GET
          await fetch(full, { method: 'GET', cache: 'no-store', mode: 'cors' });
          ok = true;
        }
        if (ok) {
        ...
        lastError = err;
        // Try next candidate; if CORS blocks 'cors' mode, try no-cors (opaque response still yields timing)
        try {
          const t0b = performance.now();
          await fetch(full, { method: 'GET', cache: 'no-store', mode: 'no-cors' });
          const t1b = performance.now();
          // mode no-cors gives opaque response but timing still valid in many browsers
          return Math.max(0, Math.round(t1b - t0b));
        } catch (err2) {
          lastError = err2;
          // continue to next
        }
  ...
  function startPingLoop() {
    // measure immediately then periodically
    const measureAndUpdate = async () => {
      try {
        ...
        if (ms === null) {
          ...
          // smooth it
          const alpha = 0.18;
          state.pingSmoothed = state.pingSmoothed ? Math.round(alpha * ms + (1 - alpha) * state.pingSmoothed) : ms;
          state.ping = state.pingSmoothed;
        }
        updateText();
      } catch (e) {
        state.ping = null;
    ...
    state.pingIntervalId = setInterval(measureAndUpdate, 2000);
  }

  // Update overlay text
  function updateText() {
    const fpsStr = (state.fps !== undefined && state.fps !== null) ? state.fps + '' : '--';
    const pingStr = (state.ping !== null && state.ping !== undefined) ? state.ping + ' ms' : '--';
    el.textContent = 'FPS: ' + fpsStr + ' | PING: ' + pingStr;
  }

  // Detect "in-game" by checking presence of `.game-view` and absence of room-link/settings, similar to original script.
  // If your Haxball page uses different selectors, change the selector below.
  function isInGame() {
    try {
      const doc = document;
      // common Haxball selectors: .game-view present when in game
      if (doc.querySelector('.game-view')) {
        // ignore if in room admin/link views:
        if (doc.querySelector('.room-view')) return false;
        if (doc.querySelector('.room-link-view')) return false;
        return true;
      }
      // fallback: some clients create a canvas with id or class; detect canvas > 0
      if (doc.querySelector('canvas')) {
        // but avoid showing in lobby pages with many canvases - only show if game-view not present and a unique canvas exists
        return true;
      }
    } catch {}
    return false;
  ...
  // Monitor page for entering/exiting game
  function startAutoToggle() {
    let lastState = false;
    const check = () => {
      const nowInGame = isInGame();
      if (nowInGame && !lastState) {
        // entered game
      ...
      } else if (!nowInGame && lastState) {
        // left game
        el.style.display = 'none';
        state.visible = false;
        // stop loops to save resources
        state.running = false;
        if (state.rafId) { cancelAnimationFrame(state.rafId); state.rafId = null; }
        if (state.pingIntervalId) { clearInterval(state.pingIntervalId); state.pingIntervalId = null; }
      }
      lastState = nowInGame;
    };
    ...
    // initial check
    check();
    // observe mutations for fast reaction
    const obs = new MutationObserver(check);
    obs.observe(document.body, { childList: true, subtree: true, attributes: false });
    // also keep a fallback interval
    state.checkIntervalId = setInterval(check, 1000);
  }

  // Start auto toggle
  startAutoToggle();

  // make functions available for debugging (optional)
  window.__hb_fps_ping_overlay._state = state;

  // immediate attempt to show if already in game
  if (isInGame()) {
    el.style.display = 'block';
    state.visible = true;
    state.running = true;
