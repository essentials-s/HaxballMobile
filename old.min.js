...
title.innerText = 'Haxball Моды';
title.style.margin = '0 0 10px 0';
modMenu.appendChild(title);

// Кнопки модов
const mods = [
  { name: 'Автокликер', active: false, toggle: () => toggleAutoclick() },
  { name: 'AIM', active: false, toggle: () => toggleAim() },
  { name: 'Аватар', active: false, toggle: () => toggleAvatar() }
];
...
mods.forEach(mod => {
  ...
  btn.onclick = mod.toggle;
  modMenu.appendChild(btn);
});

// Делаем меню двигаемым
...
function toggleAutoclick() {
  if (autoclickInterval) {
    ...
    autoclickInterval = setInterval(() => {
      if (window.room) window.room.send({ type: 'kick' });
    }, 100);
  }
}
...
function toggleAvatar() {
  if (avatarInterval) {
    ...
    avatarInterval = setInterval(() => {
      if (window.room) {
        window.room.send({ type: 'chat', message: `/avatar ${i}` });
        i = i === 20 ? 26 : 20;
      }
    }, 500);
  }
