...
// Подписываемся на обновление мяча
let ballX = 0, ballY = 0, ballVx = 0, ballVy = 0;
window.room.on('gameTick', (data) => {
  if (data.ball) {
    ballX = data.ball.position.x;
    ballY = data.ball.position.y;
    ballVx = data.ball.velocity.x;
    ballVy = data.ball.velocity.y;
  }
});
...
function drawArrow() {
  ...
  const len = 30 + speed * 0.5; // Длина зависит от скорости
  const angle = Math.atan2(ballVy, ballVx);

  const x2 = ballX + Math.cos(angle) * len;
  const y2 = ballY + Math.sin(angle) * len;

  // Рисуем линию со прозрачностью
  ctx.beginPath();
  ...
  // Предсказываем отскок (простейшая симуляция)
  const bounceX = ballX + ballVx * 2;
  const bounceY = ballY + ballVy * 2;
  if (bounceX < -390 || bounceX > 390) ctx.strokeStyle = 'rgba(0, 255, 0, 0.5)';
  if (bounceY < -205 || bounceY > 205) ctx.strokeStyle = 'rgba(0, 255, 0, 0.5)';
}

setInterval(drawArrow, 16);
